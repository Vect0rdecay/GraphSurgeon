...
export function buildShadowLogicPanel(
  ...
  scoreRow.innerHTML = `
    <span style="color:#ffb454">Exposure:</span> ${data.structural_exposure.toFixed(2)}
  ...
  for (const point of data.injection_points) {
    ...
    row.innerHTML = `
      <div style="display:flex;align-items:center;gap:6px">
        <span style="color:#ffb454;font-weight:bold;font-size:11px">${point.location}</span>
        <span style="color:#9fb8cc;font-size:9px;border:1px solid #5e7186;padding:0 3px;border-radius:2px">${point.injection_complexity}</span>
      </div>
      <div style="color:#c9d6e4;font-size:10px;margin-top:2px;word-wrap:break-word;overflow-wrap:break-word">${point.description}</div>
      <div style="color:#9fb8cc;font-size:9px;margin-top:1px">${point.node_id}</div>
    `;
...
export function showShadowLogicDetail(point: SceneShadowLogicPoint): HTMLElement {
  ...
  panel.style.cssText = `
    ...
    background: rgba(0,0,0,0.95); border: 1px solid #ffb454;
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      <span style="color:#ffb454;font-weight:bold;font-size:14px">SHADOWLOGIC POINT</span>
      <span id="sl-detail-close" style="color:#ffb454;cursor:pointer;font-size:16px;padding:0 4px">×</span>
    </div>
    <div style="margin-bottom:6px"><span style="color:#7fd4ff">Node:</span> ${point.node_id}</div>
    <div style="margin-bottom:6px"><span style="color:#7fd4ff">Location:</span> ${point.location}</div>
    <div style="margin-bottom:6px"><span style="color:#7fd4ff">Complexity:</span> ${point.injection_complexity}</div>
    <div style="margin-bottom:6px"><span style="color:#7fd4ff">Detection:</span> ${point.detection_difficulty}</div>
    <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,102,0,0.2);color:#c9d6e4;font-size:11px">${point.description}</div>
  `;
  return panel;
}
